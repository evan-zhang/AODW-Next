...
name: aodw
version: "{VERSION}"
description: AODW 开发工作流规范。触发词：RT、立项、新需求、feature 分支、spec、intake、审计、worktree。
---

# AODW-Next Kernel Loader
# 版本: {VERSION} (OpenClaw 适配)
...
## 2. 规则加载顺序 (Rule Loading Order)

AODW 规则系统采用渐进式加载策略。所有规则的元数据定义在：

- `.aodw-next/manifest.yaml` (规则索引与元数据)

**核心规则文件结构**：
- `01-core/`: 核心宪章与知识规则（最高优先级）
- `02-workflow/`: 工作流程规范（RT 管理、Spec 档案）
- `03-standards/`: 编码标准与质量规范
- `04-auditors/`: 审计官规则（需求审计、开发审计）
- `05-tooling/`: 工具初始化规则
- `06-project/`: 项目特化文件（用户生成）

**加载时机**：
- 每个规则文件定义了 `load_when` 标签（如 'init', 'plan', 'implement' 等）
- 根据当前任务阶段，仅加载相关的规则文件
- 优先级：critical > high > medium

## 3. 命令索引 (Command Index)
...
| **Spec-Lite Autopilot** | `.aodw-next/02-workflow/spec-lite-autopilot-profile.md`<br>`.aodw-next/02-workflow/autopilot-protocol.md`<br>`.aodw-next/02-workflow/rt-autopilot-readiness.md`<br>`.aodw-next/01-core/git-discipline.md` | 全自动：机械 Gate + Ralph 循环；强制 7 件套（含 autopilot-preflight.md） |
...
在 OpenClaw 平台下，本 SKILL.md 是项目级的主入口文件。相关的 Agent 规范请参考 `AGENTS.md` 文件。

**触发条件**：
- 用户明确提及 RT、立项、新需求、feature 分支、spec、intake、审计、worktree 等关键词
- 用户要求创建新的功能开发分支
- 用户要求进行代码审查或审计

**环境要求**：
- 确保项目已通过 `npx aodw-skill init` 完成初始化
- 确保 `.aodw-next/` 目录存在且包含完整的规则文件
