...
**C1 对已关闭 RT 的作用域**：触发时机既然是「冻结任务包**前**」，RT 一旦 `meta.yaml`
的 `status: done`，任务包便是历史归档件——改它是篡改交付记录，红着也无从修复，只会
让历史 RT 永久挂一条不可行动的 error。故 `status=done` 时 G112 **仍实跑扫描并如实
报出自冲突条数**（msg 形态「扫描 K 份 / 执行 N 条 / skip M 条」不变，尾部追加豁免
说明），只是不判失败。**只认 `done`**：`in-progress`/`created` 等照常硬失败，
`meta.yaml` 缺 `status` 字段亦不豁免。绕过成本：把 `status` 改成 `done` 要同时过
G111 复盘 Gate 与 G109 index 条目，且改动在 `git diff` 里可见。
