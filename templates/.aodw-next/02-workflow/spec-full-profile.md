...
### 3.2 目标、非目标与 MoSCoW 分级
- 明确列出本次 RT 的目标范围；明确**不处理**的内容（Won't = 非目标）
- 每条需求标注 MoSCoW（Constitution §1.8）：**Must = 本轮交付范围（本 RT 的 tracer bullet）**；Should / Could 按价值排序，未纳入的写入 DI 台账
- 一句话说明本轮交付的最小可用版本
