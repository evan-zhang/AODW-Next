...
5. 执行交互式澄清（选项化提问；仅 Spec-Full 或范围有歧义时必问范围切割：最小可用版本是什么？哪些延后进 DI？——Constitution §1.8）
...
遗留事项两个来源：① 过程中发现（原有）；② Spec 阶段 MoSCoW 切割主动延后（发现于标「RT-XXX（spec 切割）」，写入时点 = Spec 定稿，写入人 = 主 Agent）。红线不变：本目标 Must 未完成不得转出关 RT。
