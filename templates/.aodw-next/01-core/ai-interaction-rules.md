...
AI 所提出的所有问题必须明确属于：
	...
	•	必须提供选项
	•	必须明确推荐项**并标注强弱度**（三级，见 §1.1）
	•	必须给出推荐理由**与各选项的主要风险**（风险比优点更重要——
		只列优点的选项对比等于把分析推回给用户）
	•	必须支持 "custom: …"（≤ 5 个词）
### 1.1 推荐强弱度三级（2026-08-16 增设）

判断责任在 AI；用户的角色是提供 AI 获取不到的信息（业务背景、外部约束、偏好）
与最终否决权。据此每个推荐必须标注强弱度，且各级有强制附带项：

| 级别 | 使用判据 | 强制附带 |
|---|---|---|
| **【定论】**（坚决选） | 证据链完整，备选项有可证实的缺陷 | 「会推翻此结论的信息」一句——告诉用户什么情况下不该听 AI 的 |
| **【推荐】** | 权衡后明确倾向，备选项在特定条件下成立 | 「选备选项的合理场景」——用户对号入座即可 |
| **【倾向】** | 信息不足以定论 | 「缺什么信息、谁能补」——这是用户输入真正有价值的位置 |

配套义务：
	•	**禁止无推荐的裸选项列表**（原有条款重申）。
	•	**禁止用全「推荐」级规避表态**——证据够定论就写定论；连续多问全为
		同一强弱度时自查是否在回避判断。
	•	**用户选了非推荐项时**：先确认用户是否掌握 AI 不可见的信息——有则吸收
		重新判断，无则执行用户决定并把分歧留档（decision.md 或 rt-lite 决策记录）。
	•	**落盘义务（谁来拦）**：影响 RT 走向的决策型问题，其选项/推荐/强弱度/
		理由/用户选择须落盘该 RT 文档；收口复盘 Gate 的三方一致性核对
		（ai-knowledge-rules.md §10.2）覆盖决策留档的完整性。
