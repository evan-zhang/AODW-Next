...
#### 1.5.1 LLM 编码防失误行为（强制）

在进入代码实现前，AI 必须满足以下四项：

1. **假设显式化**：关键假设必须明确写出；
2. **歧义显式化**：存在多种解释时必须列出选项与推荐，不得静默选择；
3. **最小方案优先**：优先最小必要改动，禁止无请求功能扩展；
4. **成功标准可验证**：任务必须转写为可验证目标（测试/验收/回归路径）。
...
### 1.7 Universal Review Discipline（通用评审纪律）

> **规则**：任何审计官 / 评审 Agent（需求审计官、计划审计官、开发审计官、综合审计官，以及用户临时指派的评审角色）在产出评审意见时，必须遵守以下六条。本节与任何审计规则文件冲突时，**以本节为准**。

1. **对称禁令**：禁止无证据的肯定，也禁止无损失的挑剔。评审的目标是「让方案可靠地达成目标」，不是「找出尽可能多的问题」。技术上正确但无人受损的意见不是 finding。

2. **锚定原则**：每条 finding 必须注明它捍卫的锚点——**哪条需求 / 验收标准 / Invariant 会因它失守**。答不出失守对象的意见不得进入 findings，可降级进附录。

3. **分级与数量上限**：Blocking / Critical / Warning 三级。**Blocking + Critical 合计 ≤ 5 条**；超出时先合并同类项，仍超出的降级为 Warning。Warning 一律收进附录，不阻塞流程、不要求逐条回应。

4. **允许通过**：允许结论为「通过」。为凑数而挑出的问题本身就是流程失败。「通过」结论必须列出已检查的维度清单，证明是覆盖后的判断而非偷懒。

5. **阶段适配**：评审前先读项目阶段声明（MVP / 试运行 / 成熟；缺省按 MVP）。非成熟阶段，「未来扩展性」「以后可能需要」类意见自动降级为 Warning 附录项。

6. **反馈回路**：每轮评审后，用户明确忽略的 findings 记入 `RT/RT-XXX/audit-ignore.md`（编号 + 一句忽略理由）。下一轮评审开始前**必须先读该文件**：与已忽略意见同类的问题，无新证据不得重提。

> **增设缘由**（2026-09-04）：
> ① 外部实证：Greptile 统计 AI 评审意见中约 79% 为「技术上正确但开发者不在乎」的 nit；prompt 工程与 LLM 自评严重度均被其实测证明无效，唯一有效的降噪手段是反馈回路（记录被忽略意见并拦截同类，采纳率 19%→55%）。
> ② 本项目实证：RT-135 五轮审计 17 个 P0 中 13 个为形式问题，文档/代码比 27:1——只查正确性、不查是否过度的审计范式会系统性制造形式问题。
> ③ 已有对偶：§1.5.1 已对**执行侧**设定最小方案优先；计划审计官 v1.0 已内置减法纪律（必要性维度、删除项先行、10:1 熔断）。本节将其上升为**所有评审角色**的通用纪律，消除 04-auditors/ 内的新旧范式分裂。

---

### 1.8 Delivery-First（交付优先与持续迭代）

> **规则**：所有角色在确定范围与排期时必须遵守四条。findings 的分级与数量纪律以 §1.7 为准，本节管辖范围切割与排期。

1. **交付闭环**：产品交付型 RT 的本轮范围必须是**可独立交付、可独立验收的端到端窄路径**（tracer bullet）；文档/调研/基建/审计/批次 RT 以**可独立验收的成果**为准。验收回答「能用了什么」，不是「都处理了吗」。禁止以「当前周期一次性解决全部问题」为目标。

2. **二八切割**：Spec-Full 必须对需求逐条标注 MoSCoW（**Must = 本轮交付范围**；Should / Could 未纳入本轮的由主 Agent 在 Spec 定稿时写入 DI 台账，发现于标「RT-XXX（spec 切割）」；Won't = 非目标）。Spec-Lite 在 §1.2 写一句「最小可用交付」即视为完成切割。**红线：本 RT 的 Must 未完成不得转 DI 关闭 RT**（§3.4c/§7.3）。

3. **四象限排期**（工作项/需求排序用，不改 findings 分级）：重要 = 影响验收目标或致锚点失守；紧急 = 不处理将阻塞本轮 Must。重要且紧急 → 本轮；重要不紧急 → DI 入「会不会反复咬人」排序；不重要但紧急 → 本轮队尾；不重要且不紧急 → 丢弃或 DI。

4. **迭代默认**：优先最小可用交付，后续 RT 持续优化；「一次性全面解决」需显式论证切割不可行并经用户批准。
...
- 负责统一的 **请求票编号（RT-ID）**：`RT-{seq}`，如 `RT-001`、`RT-042`；
  - ⚠️ **重要**：RT-ID 的获取必须遵循 `.aodw-next/02-workflow/rt-id-generation-rules.md`
  - 当前策略：统一本地生成（扫描 `RT/` 目录找最大序号并递增），不依赖远程分配服务
...
1. **Spec-Full Profile**：适用复杂功能 / 大改动 / 高风险改动。详见 `.aodw-next/02-workflow/spec-full-profile.md`。
2. **Spec-Lite Profile**：适用小范围变更 / bug 修复 / 小增强。详见 `.aodw-next/02-workflow/spec-lite-profile.md`。
3. **Git Discipline**：统一的完成和收尾规则。详见 `.aodw-next/01-core/git-discipline.md`。

**知识蒸馏（完成前提）**：在执行 Git Discipline 前，必须通过 `modules-index.yaml` 找到对应模块文档，更新模块 README，确认文档反映最新代码状态。详见 `.aodw-next/01-core/ai-knowledge-rules.md` 第 4 节。
...
> **详细分析**：请参考 `.aodw-next/07-optimization/token-usage-analysis.md`
